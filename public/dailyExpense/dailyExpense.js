...
async function dailyExpense(event) {
  ...
  const token = localStorage.getItem("token");
  const { data } = await axios.post("http://localhost:3000/dailyExpense", obj, {
...
async function onScreenFunction(expense) {
  ...
  delBtn.onclick = async () => {
    await axios.delete(`http://localhost:3000/deleteExpense/${expense._id}`, {
      headers: { Authorization: token },
    });
    ul2.removeChild(li);
  };
...
async function handlePeriodicClick() {
  const token = localStorage.getItem("token");
  const { data } = await axios.get(
    "http://localhost:3000/premium/periodicExpenses",
    {
      headers: { Authorization: token },
    }
  );
  ...
  const headers = ["Period", "Total"];
  headers.forEach((header) => {
    const cell = headerRow.insertCell();
    cell.textContent = header;
  });

  function addExpensesToTable(expenses, period) {
    Object.entries(expenses).forEach(([key, expenses]) => {
      const row = newTable.insertRow();
      const periodCell = row.insertCell();
      periodCell.textContent = `${period} ${key}`;
      const totalCell = row.insertCell();
      totalCell.textContent = expenses.reduce(
        (acc, expense) => acc + expense.amount,
        0
      );
    });
  }
  addExpensesToTable(data.dailyExpenses, "Day");
  addExpensesToTable(data.monthlyExpenses, "Month");
  addExpensesToTable(data.yearlyExpenses, "Year");

  // append table to page
  periodicHtml.appendChild(newTable);
}
...
document.getElementById("rzp-button1").onclick = async function (e) {
  try {
    ...
    var options = {
      ...
      prefill: {
        email: "akshay.kumar@ekkadouble.com",
    ...
    console.log({ "problem in the dailyExpenseJs": error });
  }
};
